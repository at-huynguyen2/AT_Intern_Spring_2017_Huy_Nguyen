...
def showAllTeam hash
	...
	hash.each do |key, value|
		puts format % [ key, value.name, ((value.score == 1)?("true"):("-"))]
	end	
	puts "-----------------------------------------------------"
end
	...
	print "   1. Id team 1 :          "
		...
		loop do
			if score1 == score2
				...
				print "        * * Score of #{originHash[id2].name}: "
				addScore = gets.chomp
		...
		puts "      #{originHash[id1].name} - #{score1} : #{score2} - #{originHash[id2].name}"
		puts "      => #{originHash[idWin].name} go in round "
		Return idWin
end
